...
# Check if the list file and workdir exists
if (-not ((Test-Path -Path $filePath -PathType Leaf ) -and (Test-Path -Path $workingDir -PathType Container))) 
{
    Write-Host "Please check to make sure both $filePath and $workingDir do exist."
    exit
}
